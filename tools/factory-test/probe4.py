...
def probe_one(unit, fc, start, qty, timeout=1.2):
    try:
        ...
        return "ERR %s" % e
lines = []
for (unit, fc, start, qty) in [(1,3,0,10)]:
    lines.append("unit=%d FC%d start=%d qty=%d -> %s" % (unit, fc, start, qty, probe_one(unit, fc, start, qty)))
with open(r"D:\Code\NitroGateway\tools\factory-test\probe-result4.txt", "w") as f:
    f.write("\n".join(lines))
