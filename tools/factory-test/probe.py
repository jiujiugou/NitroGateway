...
lines = []
for (unit, fc, start, qty) in [(1,3,0,90),(1,3,86,4),(1,3,100,4),(1,4,0,6),(1,1,0,2),(1,2,0,2),(2,3,0,90),(3,3,0,90)]:
    r = probe_one(unit, fc, start, qty)
    lines.append("unit=%d FC%d start=%d qty=%d -> %s" % (unit, fc, start, qty, r))

with open(r"D:\Code\NitroGateway\tools\factory-test\probe-result.txt", "w") as f:
    f.write("\n".join(lines))
