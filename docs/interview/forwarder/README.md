...
```
Forwarder（核心循环/失败路径/固定批量上限）
→ IForwardBuffer + SqliteForwardOutbox（两阶段状态机/重试超限丢弃）
...
| 组件 | 文件 | 一句话职责 |
| --- | --- | --- |
| 转发器 | `src/NitroGateway.Forwarder/Forwarder.cs` | Dequeue→Serialize→Publish(QoS1)→Commit 核心循环，固定批量上限 MaxDequeuePerCall=1000（2026-08-22 删 AIMD） |
...
| 缓冲接口 | `src/NitroGateway.Storage/Buffer/IForwardBuffer.cs` | 两阶段 FIFO + 重试超限丢弃（纯接口；死信方法停用但保留） |
| 缓冲实现 | `src/NitroGateway.Persistence/Sqlite/SqliteForwardOutbox.cs` | SQLite 两阶段提交、启动恢复、重试超限丢弃 |
...
- `IForwardBuffer`：Storage.Buffer 纯接口；实现 `SqliteForwardOutbox`（每操作独立连接，ADR-001 P1-4）
...
- 死信处理：`GetDeadLettersAsync` / `RetryDeadLetterAsync` / `DiscardDeadLetterAsync` 已【停用】（2026-08-22 重试超限即丢弃），接口保留供未来复用
- 遥测：`NitroMetrics`（Prometheus）+ `GatewayActivitySource`（Activity "Forward"）

## 注意事项

- **代码是唯一事实来源**。`DESIGN.md` 的 v1 决策表「Dequeue 全量（maxCount=int.MaxValue）」与实现（引擎 `MaxDrainPerRound=2000` + `MaxDequeuePerCall=1000` 固定上限）不一致——答题以代码为准（Q1.6 即此类题）。2026-08-22 已删 AIMD/死信，`DESIGN.md` 部分 v1 描述为历史快照。
- 测试是理解行为最快的捷径：`tests/NitroGateway.UnitTests`（ForwarderActivityTests / SqliteForwardOutboxTests）、`tests/NitroGateway.IntegrationTests`（ForwarderIntegrationTests / ForwarderFailureTests / ForwarderEngineTests）。
- 答完全部题目后，试着不看代码把「Broker 断线 3 小时 → 恢复 → 积压排空」的完整时序画出来（缓冲状态 × 日志 × 指标）——能画出来就是吃透了。
