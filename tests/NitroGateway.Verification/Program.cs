...
Assert("60.25 变化 0.25 < 死区 0.5 → 不丢弃(设计: 死区仅影响告警缓存)", r2.Count == 1, $"实际 {r2.Count} 个");
...
var ent = NitroGateway.Persistence.DomainMapper.ToEntity(dev);
...
var back = NitroGateway.Persistence.DomainMapper.ToDomain(ent);
...
sealed class TestListener(Action<DeviceHealthChanged> onChanged) : IDeviceHealthListener
{
