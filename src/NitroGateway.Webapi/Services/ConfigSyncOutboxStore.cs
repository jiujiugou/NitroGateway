...
/// <summary>待上报变更类型（ADR-033 阶段 4：边缘改动先入 outbox，联网后按序上报中心）</summary>
public enum ConfigSyncOutboxKind
{
    /// <summary>设备 upsert（上报时取设备当前全量状态）</summary>
    Device,

    /// <summary>设备删除（tombstone 上报）</summary>
    DeviceDelete,

    /// <summary>点位 upsert（上报时取点位当前状态）</summary>
    Point,
...
/// 边缘 UI/API 每次设备/点位增删改后写入一行；同步服务上报成功后清除。
...
public interface IConfigSyncOutboxStore
{
    ...
    /// <summary>清空全部（手动对入以中心重置本地后，本地与中心一致无待上报）</summary>
    Task<OperationResult> ClearAllAsync(CancellationToken ct = default);
}

/// <summary>SQLite outbox 实现（Dapper，每操作独立连接；与桌面 ConfigSyncOutboxStore 同 schema/逻辑）</summary>
public sealed class ConfigSyncOutboxStore : IConfigSyncOutboxStore
{
    private readonly string _connectionString;

    /// <param name="connectionString">SQLite 连接串（与其他 Persistence 存储同源）</param>
