...
/// 每个周期：先拉中心快照按 UpdatedAt 双向合并下发，再清点 outbox 把边缘离线改动上报中心。
/// 未配置中心地址（ConfigSync:CenterUrl 为空）时静默跳过——仅手动导入模式；断网/鉴权失败同样跳过，
/// 不阻塞采集，下个周期补做（最终一致）。
/// </summary>
public sealed class WebConfigSyncService : BackgroundService
{
    /// <summary>轮询间隔配置键（秒，缺省 60；下限 5 防误刷屏）</summary>
    internal const string IntervalConfigKey = "ConfigSync:PollIntervalSeconds";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (true)
        {
            ...
            {
                // 同步失败绝不影响采集：静默跳过，下个周期补做（ADR-033 阶段 3）
    ...
    {
        var centerUrl = _configuration["ConfigSync:CenterUrl"]?.Trim() ?? "";
        if (string.IsNullOrWhiteSpace(centerUrl))
            return; // 未配置中心：保持手动导入模式

        var token = _configuration["ConfigSync:CenterToken"]?.Trim() ?? "";

        var snapshotResult = await _client.FetchSyncSnapshotAsync(centerUrl, token, _siteId.Current, ct);
    ...
    internal async Task PushPendingAsync(string centerUrl, string token, CancellationToken ct = default)
    {
        ...
        var pushResult = await _client.PushChangesAsync(centerUrl, token, _siteId.Current, changes, ct);
