        ...
        IReadOnlyList<PointSnapshot> snapshots, CancellationToken ct = default)
    {
        ...
        try
        {
            foreach (var s in snapshots)
            {
                ...
                _insertCmd.Parameters["@name"].Value = ""; // 后续可从 DevicePoint 获取
                ...
                _insertCmd.Parameters["@type"].Value = ""; // 后续可从 DevicePoint 获取
        ...
        catch (Exception ex)
        {
            ...
            return OperationalError.General($"时序数据写入失败: {ex.Message}");
        }
    }
