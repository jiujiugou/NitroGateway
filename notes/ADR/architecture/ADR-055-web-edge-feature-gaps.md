...
- 日期: 2026-08-18 | 状态: 已实施（2026-08-18，前端缺口补齐） | 关联: ADR-054（web 单身份化）、F-06（CSV 点位）、F-43（桌面实时曲线）
...
- 修复方向（已定）：Monitoring 页加"点选点位 → ECharts 滚动曲线"，数据源 = 现有 SignalR `Measurement` 推送；`HistoryView.vue` 已有 echarts 按需引入（Line + step:'end'）可复用。
...
- 前端：`web/src/api/devices.ts` 只有 `generatePoints` / `exportPoints`，**无 `importPoints`**；`PointList.vue` 只有"导出 CSV / 批量生成"，**无导入按钮**。
- 桌面对照：`PointsViewModel` + `CsvFileService` 有导入导出。
- 修复方向（已定）：`devices.ts` 加 `importPoints`（POST `/api/devices/{deviceId}/points/import`，`JSON.stringify(csvText)` + application/json，与既有 `updateDeviceStatus` 同款 `[FromBody] string` 编码）+ `PointList.vue` 加"⬆ 导入 CSV"按钮（tooltip 提示列头 + 隐藏 file input，复用导出按钮同排）；导入成功提示数量并刷新列表，后端错误信息透出；CSV 读取带 UTF-8/GBK 回退 + BOM 去除（中文 Excel 常导出 GBK）。
